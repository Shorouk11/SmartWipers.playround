...
class SmartWipersplayroundApp(VehicleApp):
    ...
    async def on_start(self):
        ...
        await self.publish_mqtt_event("notifyPhone", json.dumps({"result": {"message": f""""""}}))
    ...
    async def on_hood_is_open_changed(self, data: DataPointReply):
        ...
        if IsOpen:
            ...
            await self.publish_mqtt_event("notifyPhone", json.dumps({"result": {"message": f"""Info: Wipers were turned off because hood was opened"""}}))

async def main():


    logger.info("Starting SmartWipersplayroundApp...")
    vehicle_app = SmartWipersplayroundApp(vehicle)
